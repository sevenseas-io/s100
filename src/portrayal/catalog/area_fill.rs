...
impl AreaFill {
    pub(super) fn parse(node: Node) -> Result<AreaFill> {
        if node.get_name() != AREA_FILL {
...
impl ExternalFile for AreaFill {
...
mod tests {
    ...
    use super::{AreaFill, CatalogItem, FileFormat, FileType};
    ...
    fn deserialize() {
        ...
        let target = AreaFill::parse(node).unwrap();
